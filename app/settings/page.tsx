  ...
  return (
    <div className="relative min-h-dvh bg-navy px-4 py-12 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        ...
        <p className="text-sm text-white/55 mb-8">Manage your account, subscription and preferences.</p>
          ...
          <Section icon={CreditCard} title="Subscription">
            <div className="rounded-xl border border-white/10 bg-surface-1 p-4">
              <div className="flex items-start justify-between gap-3">
                ...
                {canceled ? (
                  <span className="shrink-0 rounded-full border border-amber-400/30 bg-amber-400/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-amber-300">
                    Canceling
                  </span>
                ) : (
                  <span className="shrink-0 rounded-full border border-teal-400/30 bg-teal-400/10 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-teal-300">
                    Active
                  </span>
                )}
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="flex items-center gap-2 text-sm">
                  <Calendar className="w-4 h-4 text-white/55 shrink-0" aria-hidden="true" />
                ...
                <div className="flex items-center gap-2 text-sm">
                  <CreditCard className="w-4 h-4 text-white/55 shrink-0" aria-hidden="true" />
                  <span className="text-white/50">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <button
                  ...
                  className="rounded-lg border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-white/55 cursor-not-allowed"
                >
        ...
        <Section icon={CreditCard} title={tier === "free" ? "Plan" : "Change plan"}>
          ...
          <ul className="space-y-2" aria-label="Plan options">
            {PLANS.map((p) => {
              ...
              return (
                <li key={p.key} className={`flex items-center gap-3 rounded-xl border px-4 py-3 transition ${isCurrent ? "border-teal-400/40 bg-teal-400/[0.06]" : "border-white/10 bg-surface-1"}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-baseline gap-2 flex-wrap">
                      ...
                      <span className="text-xs text-white/50 tabular-nums">
                        ...
                        <span className="text-white/55">{p.key === "free" ? " forever" : "/mo"}</span>
                      </span>
                    </div>
                    <p className="text-[11px] text-white/55 mt-0.5 leading-snug">{p.tagline}</p>
                  </div>
                      ...
                      className={`shrink-0 rounded-lg px-3 py-1.5 text-xs font-medium transition disabled:opacity-40 disabled:cursor-not-allowed ${
                        isUpgrade
                          ? p.key === "premium"
                            ...
                            : "bg-gradient-to-r from-teal-500 to-teal-400 text-navy hover:from-teal-400 hover:to-teal-400 shadow-sm shadow-teal-500/20"
        ...
        <Section icon={Globe} title="Language" comingSoon>
          <p className="text-sm text-white/55">Choose your interface language. Auto-detected from your browser by default.</p>
        </Section>
        ...
        <h2 className="mt-8 mb-3 px-1 text-xs font-semibold uppercase tracking-wider text-white/55">
          Privacy &amp; data
        </h2>
  ...
  return (
    <li className="flex items-center justify-between gap-3 py-2.5 first:pt-0">
      <div className="min-w-0">
        ...
        <div className="text-[11px] text-white/55 capitalize">{desc}</div>
      </div>
      <div className="flex items-center gap-3 shrink-0">
        <span className="rounded-full border border-teal-400/20 bg-teal-400/10 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-wider text-teal-300">Paid</span>
        <span className="text-sm text-white/80 tabular-nums">{amount}</span>
        <button type="button" disabled title="Invoice PDFs ship with Stripe" className="text-white/55 cursor-not-allowed" aria-label="Download invoice">
...
function Section({
  ...
  return (
    <section className={`mb-4 rounded-2xl border bg-surface-1 p-5 ${danger ? "border-red-400/20" : "border-white/10"} ${comingSoon ? "opacity-60" : ""}`}>
      <div className="mb-3 flex items-center gap-2">
        ...
        {comingSoon && (
          <span className="ml-auto text-[11px] font-medium uppercase tracking-wider text-white/55 border border-white/10 rounded-full px-2 py-0.5">
            Coming soon
          </span>
        )}
      </div>
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-2 first:pt-0 last:pb-0 border-b border-white/5 last:border-0">
      <div className="min-w-0">
        <div className="text-xs uppercase tracking-wider text-white/55 mb-0.5">{label}</div>
