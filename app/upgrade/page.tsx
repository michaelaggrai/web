    ...
    <div className="relative min-h-dvh flex flex-col items-center bg-navy px-4 py-16 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-4xl">
        ...
        <div className="mb-9 flex flex-col items-center gap-2">
          <div role="radiogroup" aria-label="Billing cycle" className="inline-grid grid-cols-2 gap-1 rounded-xl border border-white/10 bg-surface-1 p-1">
          ...
          {PLANS.map((plan) => {
            ...
            return (
              <PlanCard
                ...
                ring={isCurrent}
                subline={cycle === "annual" && !price.isFree ? `${price.perMonthLabel}/mo · ${price.savePctLabel}` : undefined}
              >

                {isCurrent ? (
                  <div className="rounded-xl border border-white/10 bg-white/5 py-2.5 text-center text-sm text-white/55">
                    Your current plan
                  </div>
                ) : isBelow ? (
                  <div className="rounded-xl border border-white/5 py-2.5 text-center text-sm text-white/55">
        ...
        <p className="mt-8 text-center text-xs text-white/55">
          ...
          .{" "}
          <Link href="/app" className="text-white/55 hover:text-white/70 underline underline-offset-2">
            Back to app
          </Link>
        </p>
      </div>
